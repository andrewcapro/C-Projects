...
class Queue {
 public:
  ...
  void enqueue(Customer* customer) {
    Customer* temp = new Customer;
    temp->inTime = customer->inTime;
    temp->orderTime = customer->orderTime;
    temp->waitTime = customer->waitTime;
...
int main() {
  ...
  Queue whiteCastle2;
  // dummy queues to simulate 2 counters w/ 1 real line
  ...
  while (1) {
    ...
    */
    ...
    if (whiteCastle2.head != NULL && counter1.head == NULL) {
      counter1.enqueue(whiteCastle2.head);
      whiteCastle2.dequeue(0);
    }
    // peak hrs = any time where pa is over 10%
    if (whiteCastle2.head != NULL && counter2.head == NULL && pa > 10) {
      counter2.enqueue(whiteCastle2.head);
      whiteCastle2.dequeue(0);
    }
    ...
    // counter 1
    if (counter1.head != NULL && counter1.head->orderTime == 0) {
      // process final stats before dequeue
      customerProcessed++;
      counter1.updateStats();

      // wait time: add to sum for avg + check min/max
       waitTimeSum += counter1.head->waitTime;
      if (counter1.head->waitTime > maxWaitTime) {
        maxWaitTime = counter1.head->waitTime;
        maxWaitClock = storeClock;
      }
      if (counter1.head->waitTime < minWaitTime) {
        minWaitTime = counter1.head->waitTime;
        minWaitClock = storeClock;
      }

      // serv time: add to sum for avg + check min/max
      serviceTimeSum += counter1.head->serviceTime;
      if (counter1.head->serviceTime > maxServiceTime) {
        maxServiceTime = counter1.head->serviceTime;
        maxServClock = storeClock;
      }
      if (counter1.head->serviceTime < minServiceTime) {
        minServiceTime = counter1.head->serviceTime;
    ...
    // counter 2
    if (counter2.head != NULL && counter2.head->orderTime == 0) {
      // process final stats before dequeue
      customerProcessed++;
      counter2.updateStats();

      // wait time: add to sum for avg + check min/max
      waitTimeSum += counter2.head->waitTime;
      if (counter2.head->waitTime > maxWaitTime) {
        maxWaitTime = counter2.head->waitTime;
        maxWaitClock = storeClock;
      }
      if (counter2.head->waitTime < minWaitTime) {
        minWaitTime = counter2.head->waitTime;
        minWaitClock = storeClock;
      }

      // serv time: add to sum for avg + check min/max
      serviceTimeSum += counter2.head->serviceTime;
      if (counter2.head->serviceTime > maxServiceTime) {
        maxServiceTime = counter2.head->serviceTime;
        maxServClock = storeClock;
      }
      if (counter2.head->serviceTime < minServiceTime) {
        minServiceTime = counter2.head->serviceTime;
        minServClock = storeClock;
      }
    ...
    // need to repeat to keep people properly moving and not add artificial 1 min wait period
    if (whiteCastle2.head != NULL && counter1.head == NULL) {
      counter1.enqueue(whiteCastle2.head);
      whiteCastle2.dequeue(0);
    }
    ...
    */
    if (counter1.head != NULL) {
      counter1.head->orderTime--;
    }
    if (counter2.head != NULL) {
      counter2.head->orderTime--;
    }

    // queue length: add to sum for avg + check min/max
    overallLength = whiteCastle2.size() + counter1.size() + counter2.size();
    cout << "End of minute " << storeClock << ": Line and counters have " << overallLength << " customer(s)\n";
    queueLengthSum += overallLength;
    if (overallLength > maxQueueLength) {
      maxQueueLength = overallLength;
      maxLengthClock = storeClock;
    }
    if (overallLength < minQueueLength) {
      minQueueLength = overallLength;
    ...
    if (storeClock >= 1020 && counter1.head == NULL && counter2.head == NULL) {
